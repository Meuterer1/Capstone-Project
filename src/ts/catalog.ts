...
export type Size = "S" | "M" | "L" | "XL" | "S-L" | "S, M, XL";
...
// Only run if we're on the catalog page
const catalogPageCheck = document.getElementById("catalog");
if (catalogPageCheck) {
  let items: Product[] = [];
  let currentPage = 1;
  ...
  clearFiltersButton?.addEventListener("click", () => {
    ...
    const salesStatusSelect = filterOptionsContainer?.querySelector(
      "#filterSales",
    ) as HTMLInputElement | null;
  ...
  filterForm?.addEventListener("change", () => {
    const size = (document.getElementById("filterSize") as HTMLSelectElement)
      .value;
    const color = (document.getElementById("filterColor") as HTMLSelectElement)
      .value;
    const category = (
      document.getElementById("filterCategory") as HTMLSelectElement
    ).value;
    const salesStatus = (
      document.getElementById("filterSales") as HTMLInputElement
    ).checked;
    ...
    if (catalogContainer) {
      ...
      paginatedItems.forEach((item) => {
        ...
        productCard.addEventListener("click", (e) => {
          if (
            (e.target as HTMLElement)?.classList.contains("add-to-cart-btn")
          ) {
            return;
          }
    ...
    if (topSetsContainer) {
      ...
      topSets.forEach((set) => {
        ...
        setCard.addEventListener("click", () => {
          window.location.href = `/html/product-details.html?id=${set.id}`;
        });
